  ...
  useEffect(() => {
    fetch("/api/packages")
      ...
      .then((data: ApiPackage[]) =>
        setPackages(data.filter((p) => p.active).map(toCardData))
      )
      .catch(() => {});
  }, []);
    ...
    >
      ...
      <div className="px-6 sm:px-12 lg:px-20 max-w-7xl mx-auto">
        ...
        {packages.length > 0 ? (
          ...
          <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="bg-white/[0.02] border border-white/5 rounded-sm animate-pulse" style={{ aspectRatio: "3/4" }} />
            ))}
          </div>
        )}
      </div>
